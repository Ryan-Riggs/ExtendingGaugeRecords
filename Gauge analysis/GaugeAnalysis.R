...
library(BBmisc)
################################################################################
shp = st_read("path\\to\\al##Author: Ryan Riggs
##Date: 9/12/2022
################################################################################
##Libraries.
################################################################################
...
mnQ=st_read('E:/research/RatingCurveAnalysis/zenodo/gauges.shp')
shp$meanQ=mnQ$meanQ[match(shp$Sttn_Nm, mnQ$Sttn_Nm)]
...
tm_basemap("Esri.WorldImagery")+
  # tm_shape(new[new$Sttn_Nm%!in%old$Sttn_Nm,])+
  # tm_bubbles(size=0.00005, alpha=0.5, col='red')+
  # tm_shape(old[old$Sttn_Nm%!in%new$Sttn_Nm,])+
  # tm_bubbles(size=0.00005, alpha= 0.5, col='blue')+
  tm_shape(shp)+
  tm_bubbles(size=0.0000000000005, alpha= 0.5, col='yellow')

library(tmap)
tmap_mode("view")
tm_basemap("Esri.WorldImagery")+
  tm_shape(new[new$Sttn_Nm%!in%old$Sttn_Nm&new$agency=='USGS',])+
  tm_bubbles(size=0.0000000000005, alpha=0.5, col='red')+
  tm_shape(old[old$agency=='USGS',])+
  tm_bubbles(size=0.0000000000005, alpha= 0.5, col='blue')+
  tm_shape(new[new$agency=='USGS',])+
  tm_bubbles(size=0.0000000000005, alpha= 0.5, col='yellow')
################################################################################
...
map=ggplot(world)+
  ...
  coord_sf(crs ="+proj=wag5")
map
ggsave("E:\\research\\RatingCurveAnalysis\\Figures\\Draft\\final\\Figure2_map_september_9_28.png",map,
...
ggsave("E:\\research\\RatingCurveAnalysis\\Figures\\Draft\\final\\Figure2_b_9_28.pdf",g,
...
dualPlot=ggplot(data=NULL)+ 
  ...
  guides(fill=guide_legend(nrow=1,byrow=TRUE))
dualPlot
ggsave("E:\\research\\RatingCurveAnalysis\\Figures\\Draft\\final\\Figure2_c_9_28.pdf",dualPlot,
...
out = rbindlist(openFiles)[,list(sum(N)), by=list(Sttn_Nm,type)]#[!is.na(continent)]
out$meanQ = round(shp$meanQ)[match(out$Sttn_Nm, shp$Sttn_Nm)]
out$mnQ = cut(out$meanQ, c(-10,10,100,1000,Inf))
miss = out[type=='missed']
obs = out[type=='observed']
# comb = merge(obs, miss, by=c('continent','year'))
comb = full_join(obs, miss, by=c('Sttn_Nm'))
comb$V1.x = fifelse(is.na(comb$V1.x), 0, comb$V1.x)
comb$V1.y = fifelse(is.na(comb$V1.y), 0, comb$V1.y)
comb$total = comb$V1.x+comb$V1.y
comb$perc = (comb$V1.x/(comb$V1.y+comb$V1.x))*100
comb$mnQ.x = fifelse(!is.na(comb$mnQ.x), comb$mnQ.x, comb$mnQ.y)
comb[,mean(perc),by=mnQ.x]
